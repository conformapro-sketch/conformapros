...
import { fetchEquipements, deleteEquipement, fetchTypesEquipement } from "@/lib/controles-queries";
...
const STATUT_CONFORMITE_LABELS = {
  conforme: { label: "Conforme", color: "bg-green-500" },
  non_conforme: { label: "Non conforme", color: "bg-red-500" },
  a_controler: { label: "À contrôler", color: "bg-yellow-500" },
};

export default function ControlesEquipements() {
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Équipements soumis à contrôle</h1>
          <p className="text-muted-foreground">Gestion des équipements réglementaires</p>
        </div>
                    ...
                    <TableRow key={eq.id}>
                      ...
                      <TableCell>
                        {eq.date_dernier_controle 
                          ? format(new Date(eq.date_dernier_controle), "dd/MM/yyyy", { locale: fr }) 
                          : "Jamais"}
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          <Button 
                            variant="outline" 
                            size="sm" 
                            onClick={() => { setSelectedEquipement(eq); setIsHistoryOpen(true); }}
                          >
                          ...
                          <Button 
                            variant="outline" 
                            size="sm" 
                            onClick={() => { setSelectedEquipement(eq); setIsFormOpen(true); }}
                          >
                            Modifier
                          </Button>
                          <Button 
                            variant="destructive" 
                            size="sm" 
                            onClick={() => { setEquipementToDelete(eq.id); setDeleteConfirmOpen(true); }}
                          >
                            Supprimer
                          </Button>
                        </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Cette action supprimera l'équipement et son historique de contrôles. Cette action est irréversible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
